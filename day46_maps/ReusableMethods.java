...
public class ReusableMethods {
    ...
    public static void bransOgrencisayisiYazdir(Map<Integer, String> sinifListMap) {
        ...
        Map<String,Integer> bransOgrSayiMap= new TreeMap<>();//harf sirasina gore cikmasi icin treeMap
        ...
        for (String each: valuesCollection
            ...
            valueArr=each.split(", ");
            brans=valueArr[2];
            // bransin bransOgrSayiMap'inde key olarak daha onceden
            // eklenip eklenmedigini kontrol etmeliyiz
            if (!bransOgrSayiMap.containsKey(brans))  {
                bransOgrSayiMap.put(brans,1);
            }else {
                bransdakiOgrSayisi=bransOgrSayiMap.get(brans);// get metodu int dondurdugu icin
                // key in valuesini getirir ör:(1)
                bransOgrSayiMap.put(brans,++bransdakiOgrSayisi);// her seferinde map i artirmak icin
            }
        }
